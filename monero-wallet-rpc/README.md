...
```
docker build -t monero-wallet-rpc:0.18.3.1 .
```

Available `build-arg`:

- **VRS**: monero-wallet-rpc version to install, default _0.18.3.1_
...
The `monero-wallet-cli` binary is also installed, you can use `docker run -it ghcr.io/refring/monero-containers/monero-wallet-rpc:0.18.3.1 /bin/bash` to use it stand alone or `docker exec -it {monero-wallet-rcp} /bin/bash` to enter a running container.
...
services:
  monerod:
    ...
    env:
      ...
      DIFFICULTY: 1
  monero-wallet-rpc:
    image: ghcr.io/refring/monero-containers/monero-wallet-rpc:0.18.3.1
    env:
...
docker pull ghcr.io/refring/monero-containers/monero-wallet-rpc:0.18.3.1
...
docker create -p 18083:18083\
    ...
    --env MONERO_DAEMON_ADDRESS=monerod:18081\
    --env WALLET_RPC_PORT=18083\
    ghcr.io/refring/monero-containers/monero-wallet-rpc:0.18.3.1
